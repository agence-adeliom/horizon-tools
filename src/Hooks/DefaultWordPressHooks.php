...
EOF;
    ...
    public function handleAdminBarStyles(): void
    {
        $useMainColor = self::useMainColor();
        ...
        if ($useMainColor && $useMainColorTheme) {
            ...
            $adminBarStylePath = sprintf('%s/../../resources/styles/admin-bar.css', __DIR__);
            $styleContent = file_exists($adminBarStylePath) ? file_get_contents($adminBarStylePath) : null;

            $adminBarScriptPath = sprintf('%s/../../resources/scripts/admin-bar.js', __DIR__);
            $adminBarScript = file_exists($adminBarScriptPath) ? file_get_contents($adminBarScriptPath) : null;

            if (null !== $adminBarScript) {
                echo sprintf('<script>%s</script>', $adminBarScript);
            }
